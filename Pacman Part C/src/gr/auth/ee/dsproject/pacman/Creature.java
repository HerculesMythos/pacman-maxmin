...
public class Creature implements gr.auth.ee.dsproject.pacman.AbstractCreature
{
  ...
  public int calculateNextPacmanPosition (Room[][] Maze, int[] currPosition)
  {
	double max=-1000;
	int maxi=-1;
    Node parent=new Node(currPosition[0],currPosition[1],-1,0,Maze);
    parent.setParent(null);
    createSubTreePacman(1,parent,Maze,currPosition);
    
       	for (int i=0;i<parent.getChildren().size();i++) {
       		if (max<parent.getChildren().get(i).getMaxOptions()) {
       			max=parent.getChildren().get(i).getMaxOptions();
       			maxi=i;
       		}
       	}
       	int direction=parent.getChildren().get(maxi).getMove();
       	return direction;
  }
  ...
  void createSubTreeGhosts (int depth, Node parent, Room[][] Maze, int[][] currGhostsPosition)
  {
	  ...
	  available=PacmanUtilities.allGhostMoves(Maze, currGhostsPosition);
	  double max=-1000;
	  double min=1000;
	  for (int i=0;i<available.size();i++) {
		  maze=PacmanUtilities.copy(Maze);
		  for (int j=0;j<PacmanUtilities.numberOfGhosts;j++) {
            switch (available.get(i).length) {
		       case 0 : newGhostsPosition[j][0]=currGhostsPosition[j][0];
		  		        newGhostsPosition[j][1]=currGhostsPosition[j][1]-1;
		  		        break;
		       case 1 : newGhostsPosition[j][0]=currGhostsPosition[j][0]+1;
 		                newGhostsPosition[j][1]=currGhostsPosition[j][1];
 		                break;
		       case 2 : newGhostsPosition[j][0]=currGhostsPosition[j][0];
 		                newGhostsPosition[j][1]=currGhostsPosition[j][1]+1;
 		                break;
		       default : 
		       	        newGhostsPosition[j][0]=currGhostsPosition[j][0]-1;
 		                newGhostsPosition[j][1]=currGhostsPosition[j][1];
 		                break;
	         }
		  }
		  PacmanUtilities.moveGhosts(maze,currGhostsPosition,newGhostsPosition);
